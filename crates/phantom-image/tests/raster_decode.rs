...
const WEBP_BYTES: &[u8] = include_bytes!("fixtures/rgba-2x1.webp");
...
#[test]
fn decodes_static_webp_into_rgba8() -> Result<(), Box<dyn std::error::Error>> {
    let decoder = RasterImageDecoder;
    let limits = ImageDecodeLimits::default();
    let metadata = decoder.probe(WEBP_BYTES, limits)?;
    let decoded = decoder.decode(WEBP_BYTES, limits)?;
